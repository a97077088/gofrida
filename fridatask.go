...
func NewFridaTask(d *FridaDevice, pk string, label string, args []string, starttimeout time.Duration, script string, launchmode int, Launchdelay *time.Duration, cb FridaTaskCB) *FridaTask {
	this := &FridaTask{}
	this.D = d
	this.Pk = pk
	this.Label = label
...
func (this *FridaTask) Run() error {
	...
	if this.Launchmode == LAUNCH_LAUNCH {
		...
		if this.Launchdelay != nil {
			time.Sleep(*this.Launchdelay)
		}
